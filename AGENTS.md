...
- Single responsibility per script:
  ...
  - `PlayerKit` — Kit state + countdown timer. NOTE the timer is *frame-based*, not time-based: `ApplyKit(kit, duration)` stores `duration * 60f` and `FixedUpdate` decrements it each frame (same pattern as `Offscreener`). Kits are applied via `OnCollisionEnter2D` matching GameObject tags that mirror `KitType` names (`Speed`, `Ice`, ...) — so a pickable kit object must carry the matching tag. Exposes `GetKit()`/`ApplyKit()` only.
